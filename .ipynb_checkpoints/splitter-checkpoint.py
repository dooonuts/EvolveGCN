...
class splitter():
    ...
    test
    '''
    def __init__(self,args,tasker,pred=False):
        ...
        if tasker.is_static: #### For static datsets
            ...
            if (pred == False):
                ...
                # I have no clue why link prediction uses all edges???
                if args.task == 'link_pred':
                    # dev = data_split(tasker, start, end, test = True, all_edges=True)
                    dev = data_split(tasker, start, end, test = True, all_edges=False)
                else:
                    dev = data_split(tasker, start, end, test = True)

                print("Dev Split Dates: " + str(start) + ":" + str(end))
                ...
                ## Might have to start and add the training window too? or adjust the min_time earlier
                # start = int(tasker.data.min_time) + 7
                # end   = int(tasker.max_time) + 1
                # If start=7, end=8, predicts day=9
                # 
                start = 12
                end = 13
                if args.task == 'link_pred':
                ...
                test = DataLoader(test,num_workers=args.data_loading_params['num_workers'])
                print ('Dataset test size: test', len(test))
                self.tasker = tasker
                self.test = test
