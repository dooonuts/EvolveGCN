...
class seir_dataset():
    def __init__(self,args):
        assert args.task in ['link_pred'], 'seir only implements link_pred'
        self.ecols = u.Namespace({'FromNodeId': 0,
                                  'ToNodeId': 1,
                                  'Weight': 2,
                                  'TimeStep': 3
                                })
        args.seir_args = u.Namespace(args.seir_args)

        #build edge data structure
        edges = self.load_edges(args.seir_args)
        timesteps = u.aggregate_by_time(edges[:,self.ecols.TimeStep], args.seir_args.aggr_time)
        ...
        self.feats_per_node = args.seir_args.feats_per_node
    ...
    def load_edges(self,seir_args, starting_line = 1):
        file = os.path.join(seir_args.folder,seir_args.edges_file)
        with open(file) as f:
            lines = f.read().splitlines()
        edges = [[float(r) for r in row.split(',')[0:4]] for row in lines[starting_line:]]
